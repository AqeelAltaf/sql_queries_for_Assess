ALTER VIEW [dbo].[VW_IMIS_Assess_Car_Changes]
AS
 (
...
assess_car.[IMIS Account Number]  as [Account],
...
from  BOOMI.dbo.vIMIS_Assess_CHANGES assess_car 
LEFT JOIN BOOMI_DEV.dbo.Email__c email ON assess_car.Contact_Email = email.NAME
lEFT JOIN (select acc1.*, acc2.IMIS_ID__C as [IMIS_ID]  from BOOMI_DEV.dbo.PRODAccounts acc1 left join BOOMI_DEV.dbo.PRODAccounts  acc2 on acc1.BILL_TO_PARENT__C = acc2.ID) acc ON assess_car.[IMIS Account Number] = acc.TOURISM_ID__C

LEFT JOIN IMIS.dbo.Name IMIS_name on assess_car.[IMIS Account Number] =  IMIS_name.ID) base 
