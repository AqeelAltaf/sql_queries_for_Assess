...
concat(base.[External Id],'-',base.[Account]) as [External Id]
 ...
 -- Use external id to populate this field
  assess.[IMIS Account Number] as 'Account',
  --case when Superseded = 0 and assess1.sup_flag = 2 then first_value(SEQN) OVER(partition by  assess.id, assess.Assess_year ORDER by  Superseded desc) else Null end as [Amended From],
   LAG(assess.SEQN) over(partition by assess.[IMIS Account Number], assess.Assess_Year order by assess.SEQN, assess.Superseded DESC ) as [Amended From],
...
-- new changes as of 12/12/2019
  ...
  when assess.FISCAL_MONTH = '' and assess.ASSESS_YEAR >= '2017/18' and assess.READY_TO_POST = 0   then 
      FIRST_VALUE( assess.FISCAL_MONTH ) OVER ( 
        PARTITION BY assess.[IMIS Account Number]
  ...
  --case when Superseded = 1 and assess1.sup_flag = 2 then first_value(SEQN) OVER(partition by  assess.id, assess.Assess_year ORDER by  Superseded )  else Null end as [Superseded By],
    LEAD(assess.SEQN) over(partition by assess.[IMIS Account Number], assess.Assess_Year order by assess.SEQN, assess.Superseded DESC ) as [Supersede By],
  ...
  FROM
  BOOMI.dbo.vIMIS_Assess_CHANGES assess 
   LEFT JOIN(select   [IMIS Account Number], ASSESS_YEAR,count(*) as [sup_flag] from BOOMI.dbo.vIMIS_Assess_CHANGES    group by assess_year, [IMIS Account Number] having count(*) > 1) assess1  on assess.[IMIS Account Number] = assess1.[IMIS Account Number] and assess.assess_year = assess1.assess_year 
  LEFT JOIN BOOMI_DEV.dbo.Email__c email ON assess.Contact_Email = email.NAME
  LEFT JOIN (select acc1.*, acc2.IMIS_ID__C as [IMIS_ID]  from BOOMI_DEV.dbo.PRODAccounts acc1 left join BOOMI_DEV.dbo.PRODAccounts  acc2 on acc1.BILL_TO_PARENT__C = acc2.ID) acc ON assess.[IMIS Account Number] = acc.TOURISM_ID__C
  LEFT JOIN BOOMI_DEV.dbo.IMIS_to_sf_seg_map imis_seg_map ON  assess.segment = LTRIM(imis_seg_map.code_in_imis)
  LEFT JOIN IMIS.dbo.Assess_Notice assess_notice on assess.[IMIS Account Number] = assess_notice.ID and assess.ASSESS_YEAR = assess_notice.ASSESS_YEAR
  ...
  LEFT JOIN IMIS.dbo.Name IMIS_name on assess.[IMIS Account Number] =  IMIS_name.ID) base
